...
class Collect(init):
    ...
    def filter(content,mode,exe_list=[]):
        ...
        if len(exe_list) == 1:      #表示exe_list为空
            if mode == "0":         #white mode
                '''Running in White_list mode , no shows if don't set filter APP'''
                pass
            if mode == "1":
                return content      #什么也不过滤

        elif len(exe_list) > 1:     #表示exe_list为非空
            N2 = []
...
if __name__ == "__main__":
    ...
    exe_list=[]
    with open("Session Viewer runlog_{0:}.log".format(time.strftime("%Y%m%d",time.localtime())),"a") as loginfo:
        while 1:
            exe_list.append(input("Process Name Need To Be Place in  {0:}：\n".format(mode_name)).lower())
            print(exe_list)
            if not exe_list[-1] :
        ...
        #-------------------table title-----------------------------------------
        title="{0:^20}{1:>21}      {2:<21} {3:^11} {4:^5} {5:^19} {6:^20}".format(
                                            'Log time',
                                            'Source IP',
                                            'Dest IP',
                                            'Status',
            ...
            for N1 in run.record:
                if N1[20:] not in result_store :
                    ...
                    if info == None:
                        ...
                        log ="{0:^20}{1:>21}<---->{2:<21} {3:^11} {4:^5} {5:^19} {6:^20}".format(
                                                            Log_time,
                                                            Source_IP,
                                                            # 'Source Port',
                                                            Dest_IP,
